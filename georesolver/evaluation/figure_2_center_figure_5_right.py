...
async def meshed_ping_cdns(prev_schedule: Path = None) -> None:
    """
    perform meshed pings (using all VPs) towards one IP address
...
def compute_percentile_rank(redirected_latency: float, all_latencies: list[float]):
...
def figure_2_center() -> None:
    ...
    plot_multiple_cdf(
        cdfs=cdfs,
        output_path="figure_2_center_cdns_latencies_comparison",
        metric_evaluated="rtt",
        legend_pos="lower right",
    )


def figure_5_right() -> None:
    ...
    for vp_addr, pings_per_hostname in tqdm(pings_per_vp_per_hostname.items()):
        ...
        for hostname, pings in pings_per_hostname.items():
            ...
            if (
                len(answers_per_hostname[hostname]) < 10
