...
from georesolver.agent.ecs_mapping_init import resolve_vps_subnet
...
DOCKER_IMAGE_NAME = "georesolver-agent"


def docker_run_cmd() -> str:
    return f"""
        docker run -d \
        -v "$(pwd)/results:/app/georesolver/results" \
        -v "$(pwd)/datasets/selected_hostnames.csv:/app/georesolver/datasets/selected_hostnames.csv" \
        -v "$(pwd)/datasets/internet_scale_subnets.json:/app/georesolver/datasets/internet_scale_subnets.json" \
        --network host \
        --name {DOCKER_IMAGE_NAME} \
        ghcr.io/dioptra-io/georesolver:main
    """
...
def deploy_hostname_resolution(vm: str, vm_config: dict) -> None:
    ...
    # pull docker image
    c.run("docker pull ghcr.io/dioptra-io/georesolver:main")
