...
def cbg(vp_coordinates: dict, vp_selection: list):
    """
    Find the best guess
    ...
    for vp_addr, min_rtt in vp_selection:
        lat, lon, _ = vp_coordinates[vp_addr]

        if min_rtt > 100:
            continue

        min_rtt_per_vp_ip[vp_addr] = min_rtt
    ...
    if not probe_circles:
        raise RuntimeError("for ech vps, we should have a circle")
    ...
    if len(intersections) > 2:
        ...
        centroid_lat, centroid_lon, _ = vp_coordinates[closest_vp]
