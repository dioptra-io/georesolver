...
async def get_measurement_schedule(dry_run: bool = False) -> dict:
    ...
    probing_budget = 100_000
    vps_per_target = probing_budget / len(targets)

    vps_pairwise_distance = load_json(path_settings.VPS_PAIRWISE_DISTANCE)

    vp_ids = {}
    for vp in vps:
        vp_ids[vp["addr"]] = vp["id"]
    ...
    for target in targets:
        target_vps = []
        for vp_addr in vps_pairwise_distance[target["addr"]][:vps_per_target]:
            if vp_addr == target["addr"]:
                continue

            vp_id = vp_ids[vp]
            target_vps.append(vp_id)

        for i in range(0, len(target_vps), 1):
            batch_vps = target_vps[i * batch_size : (i + 1) * batch_size]
